...
for (const width of [390, 1440]) {
  test(`Infrastructure failure, transfer and reset at ${width}px`, async ({ browser, baseURL }, testInfo) => {
    ...
    await expect(scheme.locator('[data-infra-scheme-workload]')).toContainText('node-02');
    await expect(simulation.locator('canvas, picture, img')).toHaveCount(0);
    ...
    await expect(scheme.locator('[data-infra-scheme-node="node-02"]')).toHaveAttribute('data-status', 'online');
