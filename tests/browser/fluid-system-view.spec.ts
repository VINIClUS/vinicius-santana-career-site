...
for (const project of ['cnesdata', 'limnopulse', 'infrastructure']) {
  test(`${project}: vertical cards and selection without navigation`, async ({ page }) => {
    ...
    await expect(right).toHaveCount(total);
    ...
    for (let i = 1; i < await right.count(); i++) {
      const previous = (await right.nth(i - 1).boundingBox())!;
      const current = (await right.nth(i).boundingBox())!;
