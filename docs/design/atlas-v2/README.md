...
The retained [measurement script](evidence/measure.mjs) and [generation audit](evidence/check-generation.mjs) document the original collection. Run the generation audit with `rtk proxy node docs/design/atlas-v2/evidence/check-generation.mjs` after installing dependencies and Chromium, with baseline commit `773022d` available locally. It reads the retained baseline inventory and writes a fresh `generation-report.json` inside its disposable worktree; the report and checkout paths are printed on completion. The manifest records the script's path maintenance separately from its original hash.

For measurement reruns, use `rtk proxy node docs/design/atlas-v2/evidence/measure.mjs URL OUTPUT_JSON SOURCE_SHA BUILD_ROLE`, where `BUILD_ROLE` is `baseline` or `candidate`, with a new output path and preserve the corresponding source/build inventory. The measurement script remains byte-for-byte identical to the original. Do not overwrite historical records or mix measurements from different binaries. CI runs unit, generation regression, build, static smoke and the complete browser suite for the PR commit before merge.
